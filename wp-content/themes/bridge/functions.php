...
function shortcode_multiple_maps( $atts ){
	...
	//$latest_books = get_posts( $args );
 	$catPost = get_posts($args); //Check all portfolio_page
 	$i=0;
 	$j=0;
	$map='map';
	$click='click'; 
 	$output='<div id="map" style="width: auto;height: 500px;"></div>
    <script>
    var locations = ';
   	foreach ($catPost as $post) : setup_postdata($post);
   		$j++;													//Check the data from each post
       $mykey_values = get_post_custom_values( 'latitud', $post->ID );//get_post_custom_values( string $key = '', int $post_id )
			if (sizeof($mykey_values)>0){
				foreach ( $mykey_values as $key => $value ) {
					$latitud=$value;
					if ($i>0){
						$output.=',["'.get_the_title($post->ID).'",'.$latitud.',';
					}
					else {
						$output.='[["'.get_the_title($post->ID).'",'.$latitud.',';
					}	
	 	...
	 	$mykey_values = get_post_custom_values( 'longitud', $post->ID  );
			if (sizeof($mykey_values)>0){
				foreach ( $mykey_values as $key => $value ) {
					$longitud=$value;
					$output.=$longitud.','.$i.', '.json_encode($post->post_content).']';
					$i++;
				}
	 		}
	endforeach;
	...
	$output.='
      function initMap() {
		...
		var mapOptions = {
		    zoom: 7,
		    center: new google.maps.LatLng(42.35022, -3.67527),
        ...
        /*var map = new google.maps.Map(document.getElementById("map"), {
          center: {lat: 42.35022, lng: -3.67527},
          scrollwheel: true,
          saturation:-100,
          zoom: 7
        });*/
        for (var i=0; i<locations.length; i++){
        	var shopInfo=locations[i][4]; 
        	...
        	var marker = new google.maps.Marker({
			    ...
			    map: map,
			    icon: "http://cenor2.artenovaclientes.es/wp-content/uploads/2016/11/location.png",
			    title:title,
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyC_iL-1h1jz_StV_vMbVtVfh3h2QjVUZ8c&callback=initMap" async defer></script></div>';
