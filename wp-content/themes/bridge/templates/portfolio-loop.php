	...
	<?php switch ($porftolio_template) {
		case 1: ?>
			<div class="two_columns_66_33 clearfix portfolio_container">
				<!--
				TRABALLANDO
				<?php// echo do_shortcode('[portfolio_list type="standard" hover_type_standard="default" columns="3" order_by="date" order="DESC" filter="yes" lightbox="yes" show_load_more="no" category="tiendas" show_categories="yes"]'); ?>
				<style>
				.projects_holder{
					display:none;
				}
				...
				<div >
         			<a class="showall">Mostrar Todas</a>
				</div>
				<script type='text/javascript'>

					$('.showall').click(function() { $('.projects_holder').css('display', 'block'); });
				</script>
				
				<?php

				/*$args = array(
    						'taxonomy'    => 'portfolio_category', // your custom post type Taxonomy

				);

				$categories = get_categories($args);
				echo '<div class="filter_holder">';
				echo '<ul">';
			    foreach ($categories as $cat) {
			    	if ($cat->category_parent > 0)
			        	echo '<a href="/tiendas/ "class="filter active" data-filter="'.$cat->taxonomy.'_'.$cat->term_id.'">'.$cat->name.'</a>';
